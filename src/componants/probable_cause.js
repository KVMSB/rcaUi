...
const Checklist = (props) => {
  ...
  const [gridRows, setGridRows] = useState(probabaleCause?.probable_cause_questions);
  const [rootCauseRows, setRootCauseRows] = useState(probabaleCause?.probable_cause);
  ...
  useEffect(() => {
    if (eventState && !probabaleCause &&(!gridRows || gridRows?.length==0)) {
  ...
  const rootCauseColumns = [
    ...
    {
      field: "approve", headerName: "Approve/Rule Out", width: 150,
      renderCell: (params) => (
        <RadioGroup
          row
        ...
        >
          ...
          <FormControlLabel value={"roleOut"} control={<Radio />} label={"Rule Out"} />
        </RadioGroup>
      ),
    },
